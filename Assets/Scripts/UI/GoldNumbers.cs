...
public class GoldNumbers : MonoBehaviour
{
    public static GoldNumbers Create(Vector3 position, int goldAmount)
    {
        GameObject goldNumbersPrefab = Instantiate((Resources.Load("Prefabs/NumberPopups/GoldNumbers") as GameObject), position, Quaternion.identity);
        GoldNumbers goldNumbers = goldNumbersPrefab.GetComponent<GoldNumbers>();
        goldNumbers.Setup(goldAmount);
    ...
    {
        _text.SetText("+" + damageAmount.ToString() + "G");
